...
import { Search, RotateCcw, CheckCircle2, XCircle, BookOpen, Info, SlidersHorizontal, X, ArrowLeft, Database, Layers, ChevronRight } from 'lucide-react';
...
const cleanOption = (option: string): string => {
  if (!option) return '';
  return option.replace(/^[A-Ea-e1-9]\s*(?:-|\.|\))\s*/, '').trim();
};
  ...
  const currentDeck = useMemo(() => {
    if (selectedDeckId === 'java-continuation') {
      return {
        ...
        title: 'Practice Java Continuation',
        category: 'Backend',
        description: 'Practice advanced Java threads and Project Loom virtual thread concepts.',
        questions: dumpJavaContinuation
      };
    }
    return {
      id: 'cloud-fsd',
      title: 'Practice Cloud Questions',
      category: 'Fullstack',
      description: 'Practice Cloud and Fullstack Development study questions.',
      ...
      {!selectedDeckId ? (
        <div className="space-y-12">
          ...
          <div className="text-center space-y-3 mb-4 pt-4">
            ...
            <h2 className={`text-3xl font-sans font-black tracking-tight ${theme === 'dark' ? 'text-amber-50' : 'text-zinc-900'}`}>
              Practice Cloud Study Decks
            </h2>
            <p className={`${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'} text-xs font-semibold max-w-xl mx-auto leading-relaxed`}>
              Evaluate your knowledge. Answers are hidden until clicked. Select a study deck below to get started.
            </p>
          </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <p className={`text-xs font-semibold leading-relaxed ${theme === 'dark' ? 'text-zinc-550' : 'text-zinc-400'}`}>
                    Evaluate your knowledge with Cloud and Fullstack Development study questions.
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <h3 className={`text-xl font-heading font-black leading-tight uppercase tracking-tight group-hover:text-amber-500 transition-colors ${
                    ...
                    Practice Java Continuation
                  </h3>
                  <p className={`text-xs font-semibold leading-relaxed ${theme === 'dark' ? 'text-zinc-555' : 'text-zinc-400'}`}>
                    Evaluate your knowledge with advanced Java Threading and Project Loom concepts.
                  </p>
                </div>
              </div>
        ...
        <>
          ...
          <button 
            ...
            className={`fixed left-6 top-40 z-40 h-12 px-4 border-2 rounded-2xl transition-all shadow-xl flex items-center gap-2 group cursor-pointer ${
              theme === 'dark' 
                ? 'bg-zinc-900 border-amber-500/30 text-amber-500 hover:text-amber-400 hover:border-amber-500' 
              ...
              <>
                ...
                >
                  ...
                  <div className="flex items-center justify-between pb-4 border-b border-zinc-100 dark:border-zinc-800 shrink-0">
                    <div className="flex items-center gap-2.5 text-amber-555 dark:text-amber-450">
                  ...
                  <div className="flex-1 overflow-y-auto pr-1 py-4 space-y-6 custom-scrollbar">
                      ...
                      <div className="relative group">
                        ...
                        <input
                          ...
                          className={`w-full h-10 pl-9 pr-3 rounded-xl border-2 transition-all outline-none text-[11px] font-bold ${
                            theme === 'dark'
                              ? 'bg-zinc-955 border-zinc-800 text-white focus:border-amber-500/40'
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2.5">
                        <div className={`p-3 rounded-2xl border text-center ${theme === 'dark' ? 'bg-zinc-955/60 border-zinc-800' : 'bg-zinc-50/50 border-zinc-100'}`}>
                        ...
                        <div className={`p-3 rounded-2xl border text-center ${theme === 'dark' ? 'bg-rose-955/20 border-rose-500/20' : 'bg-rose-50 border-rose-100'}`}>
                      ...
                      <div className={`p-3 rounded-2xl border flex justify-between items-center ${theme === 'dark' ? 'bg-zinc-955/60 border-zinc-800' : 'bg-zinc-50/50 border-zinc-100'}`}>
                          ...
                          <button
                            ...
                            className={`w-full px-3.5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-wider border text-left transition-all cursor-pointer flex justify-between items-center ${
                              activeSection === sec
                                ...
                                : theme === 'dark' 
                                  ? 'bg-zinc-955 border-zinc-850 text-zinc-400 hover:border-amber-500/20' 
                                  : 'bg-zinc-50 border-zinc-150 text-zinc-650 hover:bg-zinc-100'
                            }`}
                          >
                            <span>{sec}</span>
                            {activeSection === sec && <span className="h-1.5 w-1.5 rounded-full bg-zinc-955" />}
                          </button>
                        ))}
                      </div>
                    </div>
          ...
          <div className="space-y-8">
            ...
            <div className="text-center space-y-3 pt-4 max-w-2xl mx-auto">
              ...
              <p className={`${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'} text-xs font-semibold max-w-xl mx-auto leading-relaxed`}>
                {selectedDeckId === 'java-continuation'
                  ? 'Evaluate your knowledge on advanced Java Threading and Project Loom concepts. Answers are hidden until clicked.'
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center pb-3 border-b border-zinc-100/50 dark:border-zinc-800/50 mb-4">
                      <span className="flex-shrink-0 w-8 h-8 rounded-xl bg-zinc-950 dark:bg-amber-500 text-white dark:text-zinc-955 flex items-center justify-center text-xs font-black shadow-sm">
                        {i + 1}
                      </span>
                    ...
                    {hasOptions ? (
                      <div className="space-y-3">
                            ...
                            if (hasAnswered) {
                              if (isCorrectOption) {
                                buttonStyle = theme === 'dark' 
                                  ? 'border-emerald-500/40 bg-emerald-955/20 text-emerald-300 font-bold ring-2 ring-emerald-500/10' 
                                  : 'border-emerald-500 bg-emerald-50 text-emerald-900 font-bold';
                              } else if (isSelected && !isCorrectOption) {
                                buttonStyle = theme === 'dark' 
                                  ? 'border-rose-500/40 bg-rose-955/20 text-rose-300 font-bold ring-2 ring-rose-500/10' 
                        ...
                        {hasAnswered && correctOptIdx === -1 && (
                          <div className={`p-4 rounded-2xl border-2 flex items-start gap-3 mt-3 ${
                            theme === 'dark' ? 'border-emerald-500/20 bg-emerald-955/10 text-emerald-300' : 'border-emerald-100 bg-emerald-50/50 text-emerald-900'
                          }`}>
                      ...
                      <div className="space-y-3">
                        {!hasAnswered ? (
                          <button
                            ...
                            className="px-6 py-3 rounded-2xl bg-amber-500 hover:bg-amber-600 text-zinc-955 text-xs font-black uppercase tracking-widest transition-all cursor-pointer shadow-md"
                          >
                            Show Answer Key
                          </button>
                        ) : (
                    ...
                    {hasAnswered && (q.explanation || q.answer) && (
                      <div className={`mt-4 rounded-2xl border p-4 text-xs ${
                        theme === 'dark' ? 'bg-zinc-955/60 border-zinc-800 text-zinc-400' : 'bg-zinc-50 border-zinc-200 text-zinc-650'
                      }`}>
